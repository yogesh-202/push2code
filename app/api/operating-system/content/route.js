...
export async function GET(request) {
  try {
    ...
    const modules = await db.collection('osModules')
      .find({})
      .sort({ order: 1 })
      .toArray();
    ...
    const lectures = await db.collection('osLectures')
      .find({})
      .sort({ order: 1 })
      .toArray();
    ...
    const userProgress = await db.collection('osProgress')
      .find({
    ...
    console.error('Error fetching Operaing System content:', error);
    return NextResponse.json(
