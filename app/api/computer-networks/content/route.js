...
export async function GET(request) {
  try {
    ...
    const modules = await db.collection('computerNetworksModules')
      .find({})
      .sort({ order: 1 })
      .toArray();
    ...
    const lectures = await db.collection('computerNetworksLectures')
      .find({})
      .sort({ order: 1 })
      .toArray();
    ...
    const userProgress = await db.collection('computerNetworksProgress')
      .find({
    ...
    console.error('Error fetching Computer Networks content:', error);
    return NextResponse.json(
