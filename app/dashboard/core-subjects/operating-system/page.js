...
export default function ComputerNetworksPage() {
  ...
  const fetchCourseContent = async (token) => {
    try {
      const response = await fetch('/api/operating-system/content', {
        headers: {
  ...
  const handleVideoComplete = async (lectureId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/operating-system/progress', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50">
      ...
      <div className="w-full md:w-1/4 bg-white overflow-y-auto border-r h-screen">
        <div className="p-6">
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-gray-800">Operating System</h1>
